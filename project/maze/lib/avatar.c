...
static void update_maze(mazestruct_t *maze, XYPos old_pos, avatar_move *move,
	...
	//if avatar was forced to backtrack or someone else has marked the spot you just
	//left as dead, insert walls around the dead spot so no other avatars go there
...
static void get_best_move_helper(mazestruct_t *maze, Avatar *avatar,
    ...
    int bt_route = 0; //stores the route from which you just came
    int forward = 0; //stores direction out of a dead end
    int num_bt_routes = 0; //in a perfect maze, there is only 1 backtrack route
    int num_dead_spots = 0; //in a perfect maze there is only 1 route out of a dead end
	...
	//it is a backtrack
		...
		//the true backtrack direction is the direction from which you just came
		//any other direction has a wall between your current position
		int prev_direction = get_last_direction(maze, avatar->fd);
		if (direction == 3 - prev_direction) {
    ...
    //make sure to backtrack on the true backtrack route
    if (best_move->score == HAVE_TO_BACK_TRACK) {
	best_move->direction = bt_route;
    }
    //make sure to move forward to escape dead ends
 ...
 *
 * Priorities change so that rather than explore, avatar's follow the trail of 
 * 	visited spots until they all come together
 *
 * See avatar.h for details on priorities
 */
static void come_together(mazestruct_t *maze, Avatar *avatar,
